...
describe('Fixed amount coupon type', () => {
    const validator: Validator = new Validator();
    let objectToValidate: CouponTypeFixedDto;
    beforeEach(() => {
        objectToValidate = new CouponTypeFixedDto();
    ...
    describe('discount value', () => {
        describe('negative', () => {
            ...
            it('should fail if discount value is invalid', () => {
                objectToValidate.discountValue = '2USD' as any as number;
                const result: ValidationError[] = validator.validateSync(objectToValidate, validateOptions);
                expect(result).not.to.deep.equal([]);
            });
        });
    ...
    describe('applies to', () => {
        ...
        describe('SPECIFIC_PRODUCTS', () => {
            ...
            describe('negaive', () => {
                it('should not pass if applies to is SPECIFIC_PRODUCTS and appliesToProducts is invalid', () => {
                    objectToValidate.appliesTo = CouponTypeAppliedToEnum.SPECIFIC_PRODUCTS;
                    objectToValidate.appliesToProducts = ['a', NaN, true] as any[];
                    const result: ValidationError[] = validator.validateSync(objectToValidate, validateOptions);
                    expect(result).not.to.deep.equal([]);
