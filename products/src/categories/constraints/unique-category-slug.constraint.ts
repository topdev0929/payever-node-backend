...
@Injectable()
export class UniqueCategorySlugConstraint implements ValidatorConstraintInterface {
  ...
  public async validate(slug: string, args: ValidationArguments): Promise<boolean> {
    ...
    const category: CategoryModel = await this.categoryService.getBySlug(
      slug,
  ...
  public defaultMessage(args: ValidationArguments): string {
    ...
    return categoryId
      ? `Category with slug "($value)" is already exists in the category "${categoryId}" of business "${businessId}"`
      : `Category with slug "($value)" is already exists in business "${businessId}"`;
  }
...
export function IsUniqueCategorySlug(
  ...
  return (object: object, propertyName: string): void => {
    registerDecorator({
      ...
      validator: UniqueCategorySlugConstraint,
    });
  };
}
