...
import { contexts, integrations } from '@pe/builder-kit/module/migration-data';
...
async function upsert(shopDb: any, collection: string, datas: any[]): Promise<void> {
  for (let data of datas) {
    if (env.APP_NAMESPACE === 'test') {
      ...
      dataStr = dataStr.replace('.payever.org', '.test.devpayever.com');
      data = JSON.parse(dataStr);
    } else if (env.APP_NAMESPACE === 'staging') {
      let dataStr: string = JSON.stringify(data);
      dataStr = dataStr.replace('.payever.org', '.staging.devpayever.com');
      data = JSON.parse(dataStr);
    }
