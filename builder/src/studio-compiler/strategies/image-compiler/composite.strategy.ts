...
export class CompositeStrategy implements ImageCompilerStrategyInterface {
  public readonly type: ImageCompilerEffectEnum = ImageCompilerEffectEnum.COMPOSITE;
  ...
  public async runTask(
    ...
    let source: string = effect.target;
    ...
    await Jimp.read(file).then(async (image: Jimp) => {
      await Jimp.read(source).then(async (compositeImage: Jimp) => {
        image.composite(compositeImage, effect.payload.x, effect.payload.y);
      });
