...
class TestFixture extends BaseFixture {
  ...
  public async apply(): Promise<void> {
    ...
    const channelSetShop: ChannelSetModel = await this.channelSetModel.create(ChannelSetFactory.create({
      _id: channelSetIdShop,
      checkout: checkout.id,
      type: 'shop',
    }) as any);
    business.channelSets.push(channelSetShop as any);

    const channelSetPos: ChannelSetModel = await this.channelSetModel.create(ChannelSetFactory.create({
      _id: channelSetIdPos,
      checkout: checkout.id,
      type: 'pos',
    }) as any);
