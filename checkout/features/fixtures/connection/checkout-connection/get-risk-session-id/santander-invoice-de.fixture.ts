...
class TestFixture extends BaseFixture {
  ...
  public async apply(): Promise<void> {
    ...
    const connectionIdTwo: string = 'c6cb3529-9984-47bf-80f2-b9c8bb19d3a3';
    ...
    const integrationOne: IntegrationModel =
      await this.integrationModel.create(IntegrationFactory.createSantanderInvoiceDePaymentIntegration());
    ...
    connectionOne.mappedReference = connectionIdTwo;
    await connectionOne.save();
    checkout.connections.push(connectionOne);

    const integrationTwo: IntegrationModel =
      await this.integrationModel.create(IntegrationFactory.createZiniaDEBnplPaymentIntegration());
    await this.createSubscription(business, integrationTwo);
    const connectionTwo: ConnectionModel = await this.createConnection(business, integrationTwo, connectionIdTwo);
