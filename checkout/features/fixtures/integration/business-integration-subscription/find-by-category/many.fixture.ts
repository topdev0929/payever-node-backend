...
class TestFixture extends BaseFixture {
  ...
  public async apply(): Promise<void> {
    ...
    const paymentsIntegrations: IntegrationModel[] = await this.integrationModel.create(
      IntegrationFactory.createPaymentsIntegrations(),
    );
    await this.pushToBusiness(paymentsIntegrations, business);

    const shippingsIntegrations: IntegrationModel[] = await this.integrationModel.create(
      IntegrationFactory.createShippingsIntegrations(),
    );
    await this.pushToBusiness(shippingsIntegrations, business);

    const shopsystemsIntegrations: IntegrationModel[] = await this.integrationModel.create(
      IntegrationFactory.createShopsystemsIntegrations(),
    );
    await this.pushToBusiness(shopsystemsIntegrations, business);

    await business.save();
  }

  private async pushToBusiness(
    paymentIntegrations: IntegrationModel[],
