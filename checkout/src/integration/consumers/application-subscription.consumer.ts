...
import { ToggleApplicationSubscriptionDto } from '../dto/toggle-application-subscription.dto';
...
@Controller()
export class ApplicationSubscriptionConsumer {
  constructor(
  ...
  public async onBusinessApplicationInstalled(toggleDto: ToggleApplicationSubscriptionDto): Promise<void> {
    await validate(toggleDto);

    const integration: IntegrationModel = await this.integrationService.findOneByName(toggleDto.code);
    if (!integration) {
      return;
    }
  ...
  public async onBusinessApplicationUninstalled(toggleDto: ToggleApplicationSubscriptionDto): Promise<void> {
    await validate(toggleDto);
