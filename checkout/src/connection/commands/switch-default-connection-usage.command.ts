...
@Injectable()
export class SwitchDefaultConnectionUsageCommand {
  constructor(
  ...
  @Command({
    command: 'checkout:connection:use-default [--connection_id] [--business_id] [--integration] [--enabled]',
    describe: 'Always use default variant in checkout if enabled',
  })
  public async export(
    ...
    if (connectionId) {
      ...
      await this.processSingleConnection(connection, enabled);
  ...
  private async processSingleConnection(
    ...
    options.useDefaultVariant = enabled;
