...
async function addDefaults(): Promise<void> {
  const appsByType: { [key: string]: DashboardApp[] } = await getAppsByType();
...
async function getAppsByType(): Promise<{ [key: string]: DashboardApp[] }> {
  const apps: DashboardAppModel[] = await dashboardAppModel.find();
