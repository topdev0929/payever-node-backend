...
export class ChannelEventMessagesProducerMock extends AbstractMessageMock {
  ...
  public async mockChannelSetCreated(): Promise<void> {
    const producer: ChannelEventMessagesProducer = await this.getProvider<ChannelEventMessagesProducer>(
      ChannelEventMessagesProducer,
    );
    await producer.sendChannelSetCreatedMessage(channelSetModelFake, channelAwareBusinessModelFake._id, channelModelFake);
  }
  ...
  public async mockChannelSetCreatedByDefault(): Promise<void> {
    ...
    await producer.sendChannelSetCreatedByDefault(channelSetModelFake, uuid.v4());
  }
  ...
  public async mockChannelSetNamed(): Promise<void> {
    ...
    await producer.sendChannelSetNamedByApplication(channelSetModelFake, 'testName');
  }
  ...
  public async mockChannelSetActivated(): Promise<void> {
    ...
    await producer.sendChannelSetActivated(channelSetModelFake, channelAwareBusinessModelFake);
  }
}
