...
class UpdateProductIntegrationsEnabledFixture extends BaseFixture {
  ...
  public async apply(): Promise<void> {
    ...
    const connectionStripe: ConnectionModel = await this.connectionModel.create({
    ...
    const connectionPaypal: ConnectionModel = await this.connectionModel.create({
    ...
    await this.connectionPlanModel.create(PlanFactory.create({
      _id: PLAN_ID_2,
      businessId: BUSINESS_ID,
      connection: connectionPaypal,
      subscriptionPlan: SUBSCRIPTION_PLAN_ID,
    }));
  }
}

export = UpdateProductIntegrationsEnabledFixture;
