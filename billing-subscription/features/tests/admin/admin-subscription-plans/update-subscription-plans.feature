Feature: Update subscription plans
  ...
  Scenario: Received request for update subscription plans
    ...
    When I send a PUT request to "/api/admin/subscription-plans/{{subscriptionPlanId}}" with json:
      """
      {
        "billingPeriod": 1,
        "interval": "week",
  ...
  Scenario: Set subscription plans as default
    Given I use DB fixture "subscription-plans/delete-subscription-plan"
    When I send a PATCH request to "/api/admin/subscription-plans/{{subscriptionPlanId}}/set-default"
    Then print last response
    And the response status code should be 200
    And the response should contain json:
    """
