Feature: Delete subscription plan for product
  Background:
  ...
  Scenario: Received request for delete subscription plans, without authentication
    ...
    When I send a DELETE request to "/api/business/{{businessId}}/subscription-plans/{{subscriptionPlanId}}"
  ...
  Scenario: Received request for delete subscription plans to another businessId
    Given I authenticate as a user with the following data:
      """
      {
        "email": "email@email.com",
      ...
      """
    Given I use DB fixture "subscription-plans/delete-subscription-plan"
    When I send a DELETE request to "/api/business/{{businessId2}}/subscription-plans/{{subscriptionPlanId}}"
    Then print last response
    And the response status code should be 403

  Scenario: Received request for delete subscription plans to another user businessId
    Given I authenticate as a user with the following data:
      """
      {
        "email": "email@email.com",
