Feature: Admin user media features
  Background:
  ...
  Scenario: Only admin role has access to admin endpoint
    Given I authenticate as a user with the following data:
      ...
      """
    When I send a GET request to "/api/admin/user-medias"
    Then response status code should be 403

  Scenario: Create user media
    When I send a POST request to "/api/admin/user-medias" with json:
      """
      {
  ...
  Scenario: Update user media
    When I send a PATCH request to "/api/admin/user-medias/{{userMediaId}}" with json:
      """
      {
        "url":"https://example.com/someimage.png",
        "mediaType": "image",
  ...
  Scenario: Get user media    
    When I send a GET request to "/api/admin/user-medias?businessIds={{businessId}}&page=1&limit=3"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Search user media by name
    ...
    When I send a GET request to "/api/admin/user-medias?page=1&limit=3&name= 1"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Get user media by id
    When I send a GET request to "/api/admin/user-medias/{{userMediaId}}"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Get user media by album id
    ...
    When I send a GET request to "/api/admin/user-medias?albumIds={{albumId1Lvl0}}"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Delete user media by id
    When I send a DELETE request to "/api/admin/user-medias/{{userMediaId}}"
    Then print last response
    Then the response status code should be 200
  ...
  Scenario: Get user media by user attribute
    ...
    When I send a GET request to "/api/admin/user-medias?userAttributeId={{userAttributeId}}&userAttributeValue=ford"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Get user media by multiple user attribute
    Given I get file "features/data/get-user-media-by-user-attribute.response.json" content and remember as "response" with placeholders
    When I send a POST request to "/api/admin/user-medias/find-by-multiple-user-attributes/business/{{businessId}}" with json:
      """
      {
        "attributes": [
          {
