Feature: Admin attribute group features
  Background:
  ...
  Scenario: Only admin role has access to admin endpoint
    Given I authenticate as a user with the following data:
      ...
      """
    When I send a GET request to "/api/admin/user-attribute-groups"
    Then response status code should be 403

  Scenario: Create user attribute group
    When I send a POST request to "/api/admin/user-attribute-groups" with json:
      """
      {
  ...
  Scenario: Update user attribute group
    When I send a PATCH request to "/api/admin/user-attribute-groups/{{attributeGroupId}}" with json:
      """
      {
        "businessId": "{{businessId}}",
        "name": "Detail Media"
  ...
  Scenario: Get user attribute group
    When I send a GET request to "/api/admin/user-attribute-groups?page=1&limit=3"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Get user attribute group by id
    When I send a GET request to "/api/admin/user-attribute-groups/{{attributeGroupId}}"
    Then print last response
    Then the response status code should be 200
    And the response should contain json:
      """
  ...
  Scenario: Delete user attribute group by id
    When I send a DELETE request to "/api/admin/user-attribute-groups/{{attributeGroupId}}"
    Then print last response
    Then the response status code should be 200
