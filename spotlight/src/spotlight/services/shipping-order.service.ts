...
@Injectable()
export class ShippingOrderService extends BaseService {
  ...
  public async search(
    ...
    if (user.roles.findIndex((role: UserRoleInterface) => role.name === RolesEnum.admin) === -1) {
      elasticFilters = this.applyBusiness('business', business._id, elasticFilters);
    }
  ...
  private async selectIdentifiersByFilter(
    ...
    return this.elasticSearchClient.search(
      ElasticIndexEnum.ShippingOrders,
      body,
    )
      .then((results: any) => {
