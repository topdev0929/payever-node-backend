...
import { ReportDetail, ReportDetailDocument } from '../schemas';
import { ProcessingStatus } from '../enum/processing-status.enum';

@Injectable()
export class ReportDetailService {
  constructor(
    @InjectModel(ReportDetail.name) private readonly reportDetailModel: Model<ReportDetailDocument>,
  ) { }

  public async create(dto: Partial<ReportDetail>): Promise<any> {
