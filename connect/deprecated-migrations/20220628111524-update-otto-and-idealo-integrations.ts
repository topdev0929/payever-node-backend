...
const integrationNames: string[] = ['otto', 'idealo'];

async function up(db: any): Promise<void> {
  const client: MongoClient =
    new MongoClient(db.connectionString, { useNewUrlParser: true, useUnifiedTopology: true });
  ...
  for (const integrationName of integrationNames) {
    const fixture: any =
      integrationsFixture.find((integration: any) => integration.name === integrationName);
    console.log(fixture);
