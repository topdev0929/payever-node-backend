...
import { MissingTransactionsCron } from './checkout-analytics/cron';
...
async function bootstrap(): Promise<void> {
  ...
  const app: INestMicroservice = await NestFactory.createMicroservice(ApplicationModule, {
    strategy: context.get(MissingTransactionsCron),
  });

  const logger: NestKitLogger = app.get(NestKitLogger);
  app.useLogger(logger);

  app.listen(() => logger.log(`Missing transactions cron started`, 'NestApplication'));
}
