...
(async () => {
  ...
  const socket = io('ws://localhost:9940/chat', {
    path: '/ws',
  ...
  socket.on('authenticated', () => {
    ...
    (async () => {
      const chats = await (await fetch(`${apiUrl}/business/${businessId}/messaging`, {
        headers: new Headers({
      ...
      for (const chat of chats) {
        ...
        const scrollId = UUIDGeneratorBrowser();

        socket.emit('messages.ws-client.message.scroll.request', {
          _id: scrollId,
          chat: chat._id,
        });
      }
  ...
  socket.on('messages.ws-client.list.message.updated', function (data) {
    //  list messages was updated
  });
  ...
  socket.on('messages.ws-client.scroller.response', function (scrollResponse) {
    if (!scrollResponse.messages.length || !scrollResponse.hasNext) { return; }
    socket.emit('messages.ws-client.message.scroll.request', {
      _id: scrollResponse._id,
      chat: scrollResponse.chat,
    });
  });

  function appendMessage(message) {
    chat.textContent += `${message.sentAt}: ${message.sender}: ${message.content} (${message._id})\n`;
  }
  ...
  function deleteFn() {
    const idToDelete = idtodelete.value;
    socket.emit('messages.ws-client.message.delete', {
      _id: idToDelete,
