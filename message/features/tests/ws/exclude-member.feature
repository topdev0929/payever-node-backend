Feature: Handle exclude user
  ...
  Scenario: Force leave sockets on exclude user
    ...
    Then I emit socket-io event "messages.ws-client.chat.exclude-member" with json:
      """
     {
    ...
    Then I look for last socket-io event "messages.ws-client.chat.member.excluded" and remember as "exclude_event_data"
    And print storage key "exclude_event_data"
    Then stored value "exclude_event_data" should contain json:
      """
      {
        "chatId": "{{ID_OF_CUSTOMER_CHAT}}",
        "user": {
