...
export class ProductWallpaperFilterByNameService implements ProductWallpaperFilterInterface {
  public async filter(
    wallpaper: ProductWallpapersDto,
    condition: SearchDto,
  ): Promise<boolean> {
    if (condition.filter !== ProductWallPaperFilterEnum.Name) {
      return true;
    }
    ...
    if (
      (
        ...
        && wallpaper.name?.toLowerCase().includes(condition.searchText.toLowerCase())
      )
      || (
