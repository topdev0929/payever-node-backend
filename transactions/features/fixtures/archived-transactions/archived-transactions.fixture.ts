import { getModelToken } from '@nestjs/mongoose';
import { Model } from 'mongoose';

import { BaseFixture } from '@pe/cucumber-sdk';
import {
  ArchivedTransactionModel,
  ArchivedTransactionSchemaName,
} from '../../../src/archived-transactions';
import { PaymentStatusesEnum } from '../../../src/transactions/enum';
import { archivedTransactionFactory } from '../factories';

const archivedTransactionId: string = '16dd795c-3ead-48fd-9423-0479297a887c';
const businessId: string = '36bf8981-8827-4c0c-a645-02d9fc6d72c8';
const userId: string = '08a3fac8-43ef-4998-99aa-cabc97a39261';
const businessOptionId: number = 1;
const paymentFlowId: string = '2';

class ArchivedTransactionsFixture extends BaseFixture {

  private readonly archivedTransactionModel: Model<ArchivedTransactionModel> =
    this.application.get(getModelToken(ArchivedTransactionSchemaName));

  public async apply(): Promise<void> {

    await this.archivedTransactionModel.create(archivedTransactionFactory.create({
      _id: archivedTransactionId,
      uuid: 'ad738281-f9f0-4db7-a4f6-670b0dff5327',
      businessId: businessId,
      data: {
        uuid: archivedTransactionId,
        _id: '7ec33a43-2fd7-4193-b57f-14b144b6c4a8',
        total: 50,
        type: 'payex_creditcard',
      },
      encryptedData: ''
    }));
  }
}

export = ArchivedTransactionsFixture;
